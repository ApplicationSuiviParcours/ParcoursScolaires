...
@section('title', 'Gestion des affectations Enseignant-Matière-Classe')
...
<div class="container mx-auto px-4 py-10 bg-gray-50">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-indigo-500 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500 font-medium uppercase">Total affectations</p>
            ...
            <div class="mt-2 text-xs text-gray-500">
                <span class="font-medium">{{ $stats['total_enseignants'] }}</span> enseignants distincts
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-blue-500 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500 font-medium uppercase">Matières enseignées</p>
                    <p class="text-3xl font-bold text-gray-800">{{ $stats['total_matieres'] }}</p>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-yellow-500 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500 font-medium uppercase">Années scolaires</p>
                    <p class="text-3xl font-bold text-gray-800">{{ $stats['total_annees'] }}</p>
                </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg p-6 mb-8" x-data="{ filters: {{ request()->anyFilled(['search', 'enseignant_id', 'classe_id', 'matiere_id', 'annee_scolaire_id']) ? 'true' : 'false' }} }">
        ...
        <form method="GET" action="{{ route('admin.enseignant_matiere_classes.index') }}" class="space-y-4">
            ...
            <div x-show="filters" x-transition.duration.300 class="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4">
                ...
                <div>
                    ...
                    <select name="enseignant_id" class="w-full rounded-xl border-2 border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 px-4 py-2">
                ...
                <div>
                    ...
                    <select name="classe_id" class="w-full rounded-xl border-2 border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 px-4 py-2">
                ...
                <div>
                    ...
                    <select name="matiere_id" class="w-full rounded-xl border-2 border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 px-4 py-2">
                ...
                <div>
                    ...
                    <select name="annee_scolaire_id" class="w-full rounded-xl border-2 border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 px-4 py-2">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gradient-to-r from-indigo-50 to-purple-50">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-indigo-700 uppercase tracking-wider">Enseignant</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-indigo-700 uppercase tracking-wider">Matière</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-indigo-700 uppercase tracking-wider">Classe</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-indigo-700 uppercase tracking-wider">Année scolaire</th>
                        ...
                        <tr class="hover:bg-indigo-50/50 transition-colors duration-200 group">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        <div class="text-xs text-gray-500">
                                            {{ $affectation->enseignant->specialite ?? 'Aucune spécialité' }}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center mr-2">
                                        <span class="text-blue-700 font-bold text-xs">{{ substr($affectation->matiere->code ?? 'MT', 0, 3) }}</span>
                                    </div>
                                    <span class="text-sm font-medium text-gray-900">{{ $affectation->matiere->nom }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('admin.enseignant_matiere_classes.show', $affectation) }}" 
                                    ...
                                    <a href="{{ route('admin.enseignant_matiere_classes.edit', $affectation) }}" 
                                    ...
                                    <form action="{{ route('admin.enseignant_matiere_classes.destroy', $affectation) }}" 
                                          method="POST" 
                                          class="inline"
                                          onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette affectation ? Cette action est irréversible.');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12">
                                <div class="flex flex-col items-center justify-center text-center">
                                    ...
                                    <h3 class="text-xl font-bold text-gray-800 mb-2">Aucune affectation trouvée</h3>
        ...
        @if($affectations->hasPages())
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex items-center justify-between">
                <div class="text-sm text-gray-600">
                    Affichage de <span class="font-medium">{{ $affectations->firstItem() }}</span> à 
                    <span class="font-medium">{{ $affectations->lastItem() }}</span> sur 
                    <span class="font-medium">{{ $affectations->total() }}</span> affectations
                </div>
                <div class="flex items-center space-x-2">
