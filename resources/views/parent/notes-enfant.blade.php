...
<div class="py-6 md:py-12">
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="relative p-4 md:p-8">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                    <div class="flex items-center space-x-4 md:space-x-6">
                        <!-- Avatar élève avec effet 3D - version compacte sur mobile -->
                        <div class="relative group perspective">
                            <div class="relative transition-all duration-500 transform group-hover:rotate-y-180 preserve-3d">
                                <div class="flex items-center justify-center w-16 h-16 md:w-24 md:h-24 transition-all duration-300 border-4 shadow-2xl bg-white/20 backdrop-blur-xl rounded-xl md:rounded-2xl border-white/50 group-hover:scale-110 group-hover:shadow-emerald-500/50">
                        ...
                        <div class="transition-all duration-700 transform animate-slide-in-left">
                            <h3 class="text-xl md:text-4xl font-black tracking-tight text-white drop-shadow-xl">
                                Notes de {{ $eleve->prenom }} {{ $eleve->nom }}
                            </h3>
                            <div class="flex flex-wrap items-center gap-2 md:gap-3 mt-2 md:mt-3">
                                <!-- Badge classe -->
                                <span class="inline-flex items-center px-2 md:px-4 py-1 md:py-2 text-xs md:text-sm font-medium text-white transition-all border rounded-full bg-white/20 backdrop-blur-sm border-white/30 hover:bg-white/30">
                                ...
                                <span class="inline-flex items-center px-2 md:px-4 py-1 md:py-2 text-xs md:text-sm font-bold text-white border rounded-full bg-emerald-500/80 backdrop-blur-sm border-white/30">
                                    <svg class="w-3 h-3 md:w-4 md:h-4 mr-1 md:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="hidden lg:block transition-all duration-700 transform animate-float-rotate hover:rotate-12 hover:scale-110">
        ...
        <div class="mb-4 md:mb-6">
            <a href="{{ route('parent.enfants') }}" class="group inline-flex items-center px-3 md:px-5 py-2 md:py-2.5 text-sm md:text-base text-gray-700 bg-white/80 backdrop-blur-sm hover:bg-white rounded-lg md:rounded-xl transition-all duration-300 shadow-md hover:shadow-xl border border-gray-200/50">
                <svg class="w-4 h-4 md:w-5 md:h-5 mr-2 transition-transform group-hover:-translate-x-1 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                </svg>
                <span class="font-medium">Retour à mes enfants</span>
                <span class="hidden md:inline ml-2 px-2 py-0.5 text-xs bg-emerald-100 text-emerald-700 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">Tableau de bord</span>
            </a>
        </div>

        <!-- Filtres modernes avec glassmorphism - responsive -->
        <div class="mb-6 md:mb-8 overflow-hidden transition-all duration-500 border shadow-xl bg-white/90 backdrop-blur-xl border-white/50 rounded-xl md:rounded-2xl hover:shadow-2xl animate-fade-in-up">
            <div class="p-4 md:p-6 border-b border-gray-100/50 bg-gradient-to-r from-emerald-50/50 to-teal-50/50">
                <div class="flex items-center justify-between">
                    <h3 class="flex items-center text-base md:text-lg font-semibold text-gray-900">
                        <svg class="w-4 h-4 md:w-5 md:h-5 mr-2 text-emerald-600 animate-spin-slow" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path>
                        </svg>
                        Filtrer les notes
                    </h3>
                    <span class="px-2 md:px-3 py-1 text-xs text-gray-500 rounded-full bg-white/50">Recherche avancée</span>
                </div>
            </div>
            <div class="p-4 md:p-6">
                <form method="GET" action="{{ route('parent.enfant.notes', $eleve->id) }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4" id="filterForm">
                    <div class="space-y-2">
                        ...
                        <select name="matiere_id" class="w-full p-2  text-sm transition-all border-gray-200 rounded-lg md:rounded-xl focus:border-emerald-500 focus:ring-emerald-500 hover:border-emerald-300">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="periode" class="w-full p-2  text-sm transition-all border-gray-200 rounded-lg md:rounded-xl focus:border-emerald-500 focus:ring-emerald-500 hover:border-emerald-300">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="date" name="date_debut" value="{{ request('date_debut') }}" 
                               class="w-full p-2 text-sm transition-all border-gray-200 rounded-lg md:rounded-xl focus:border-emerald-500 focus:ring-emerald-500 hover:border-emerald-300">
                    </div>
                    ...
                    <div class="flex items-end space-x-2">
                        <a href="{{ route('parent.enfant.notes', $eleve->id) }}" 
                           class="flex-1 px-3 md:px-4 py-2 md:py-3 font-medium text-center text-gray-600 transition-all border bg-gray-100/80 backdrop-blur-sm rounded-lg md:rounded-xl hover:bg-gray-200 hover:shadow-md border-gray-200/50">
        ...
        <!-- Stats Cards avec design moderne et animations - responsive grid -->
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6 md:mb-8">
            <!-- Total notes -->
            <div class="relative p-3 md:p-6 overflow-hidden transition-all duration-500 border shadow-xl group bg-white/90 backdrop-blur-xl border-white/50 rounded-xl md:rounded-2xl hover:shadow-2xl hover:-translate-y-1">
            ...
            <div class="relative p-3 md:p-6 overflow-hidden transition-all duration-500 border shadow-xl group bg-white/90 backdrop-blur-xl border-white/50 rounded-xl md:rounded-2xl hover:shadow-2xl hover:-translate-y-1">
                ...
                <div class="relative flex items-center justify-between">
                    <div>
                        <p class="text-xs md:text-sm font-medium text-gray-600">Moyenne générale</p>
                        <p class="mt-1 md:mt-2 text-xl md:text-4xl font-black {{ $stats['moyenne_generale'] >= 10 ? 'text-emerald-600' : 'text-red-500' }} group-hover:scale-110 transition-transform origin-left">
            ...
            <div class="relative p-3 md:p-6 overflow-hidden transition-all duration-500 border shadow-xl group bg-white/90 backdrop-blur-xl border-white/50 rounded-xl md:rounded-2xl hover:shadow-2xl hover:-translate-y-1">
                ...
                <div class="relative flex items-center justify-between">
                    <div>
                        <p class="text-xs md:text-sm font-medium text-gray-600">Note minimale</p>
            ...
            <div class="p-4 md:p-6 bg-gradient-to-r from-emerald-600 via-teal-500 to-cyan-600">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                    ...
                    <div class="flex items-center space-x-2 md:space-x-3">
                        <span class="px-2 md:px-4 py-1 md:py-2 text-xs md:text-sm font-bold text-white border rounded-full bg-white/20 backdrop-blur-sm border-white/30">
                            {{ $stats['total_notes'] }} note(s)
                        </span>
                        <button onclick="window.print()" class="p-1.5 md:p-2 text-white transition-all rounded-lg bg-white/20 backdrop-blur-sm hover:bg-white/30">
                ...
                <table class="min-w-full md:w-full">
                    <thead class="bg-gradient-to-r from-gray-50 to-gray-100/80">
                        <tr>
                            ...
                            <th class="px-3 md:px-6 py-3 md:py-4 text-xs font-bold tracking-wider text-left text-gray-600 uppercase whitespace-nowrap">Type</th>
                            <th class="px-3 md:px-6 py-3 md:py-4 text-xs font-bold tracking-wider text-left text-gray-600 uppercase whitespace-nowrap">Note</th>
                            <th class="px-3 md:px-6 py-3 md:py-4 text-xs font-bold tracking-wider text-left text-gray-600 uppercase whitespace-nowrap">Coeff.</th>
                            <th class="px-3 md:px-6 py-3 md:py-4 text-xs font-bold tracking-wider text-left text-gray-600 uppercase whitespace-nowrap">Appréciation</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="transition-all duration-300 cursor-pointer group hover:bg-gradient-to-r hover:from-emerald-50/50 hover:to-teal-50/50">
                                <td class="px-3 md:px-6 py-3 md:py-5 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-1.5 h-1.5 md:w-2 md:h-2 mr-2 md:mr-3 rounded-full bg-emerald-500 group-hover:animate-pulse"></div>
                                ...
                                <td class="px-3 md:px-6 py-3 md:py-5">
                                    <div class="relative inline-block">
                                        <span class="px-2 md:px-5 py-1.5 md:py-2.5 text-xs md:text-sm font-black rounded-lg md:rounded-xl bg-gradient-to-r {{ $noteClass }} text-white shadow-lg group-hover:shadow-xl transition-all whitespace-nowrap">
                                            {{ number_format($noteValue, 2) }}/20
                                        </span>
                                        <span class="absolute w-2 h-2 md:w-3 md:h-3 bg-green-400 border-2 border-white rounded-full -top-1 -right-1 animate-pulse"></span>
                                    </div>
                                </td>
                                <td class="px-3 md:px-6 py-3 md:py-5">
                                    <span class="px-2 md:px-4 py-1 md:py-2 text-xs md:text-sm font-bold text-orange-700 bg-orange-100 border border-orange-200 rounded-full whitespace-nowrap">
                                        x{{ $note->evaluation->coefficient ?? 1 }}
                                    </span>
                                </td>
                                <td class="px-3 md:px-6 py-3 md:py-5 max-w-[150px] md:max-w-xs">
                                    @if($appreciation != '-')
                                        <div class="relative group/tooltip">
                                            <div class="flex items-center space-x-1 md:space-x-2">
                                                <svg class="flex-shrink-0 w-3 h-3 md:w-4 md:h-4 text-emerald-500" fill="currentColor" viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"></path>
                                                </svg>
                                                <span class="text-xs md:text-sm italic font-medium text-gray-700 truncate transition-colors group-hover:text-emerald-600">
                                                    "{{ Str::limit($appreciation, 30) }}"
                                                </span>
                                            </div>
                                            <div class="absolute left-0 z-10 hidden w-48 md:w-64 p-2 md:p-3 mb-2 text-xs md:text-sm text-white bg-gray-900 rounded-lg shadow-xl bottom-full group-hover/tooltip:block">
                                                {{ $appreciation }}
                                            </div>
                                        </div>
                                    @else
                                        <span class="flex items-center text-xs md:text-sm text-gray-400 whitespace-nowrap">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-3 md:px-6 py-8 md:py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        <svg class="w-16 h-16 md:w-24 md:h-24 mb-3 md:mb-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mt-6 md:mt-8 overflow-hidden border shadow-xl bg-white/90 backdrop-blur-xl border-white/50 rounded-xl md:rounded-2xl">
            ...
            <div class="p-4 md:p-6">
                <div class="flex items-end h-48 md:h-64 space-x-1 md:space-x-2">
                    @php
                    ...
                    @foreach($recentNotes as $index => $note)
                        ...
                        <div class="flex flex-col items-center flex-1 group">
                            <div class="relative w-full">
                                <div class="w-full h-48 md:h-64 overflow-hidden bg-gray-100 rounded-t-lg">
                                    <div class="{{ $color }} transition-all duration-500 group-hover:opacity-80" 
                                         style="height: {{ $height }}%; width: 100%;"></div>
                                </div>
                                <div class="absolute px-1.5 md:px-2 py-0.5 md:py-1 text-[10px] md:text-xs text-white transition-opacity transform -translate-x-1/2 bg-gray-900 rounded opacity-0 -top-6 md:-top-8 left-1/2 group-hover:opacity-100 whitespace-nowrap">
                                    {{ number_format($note->note, 2) }}/20
                                </div>
                            </div>
                            <span class="mt-1 md:mt-2 text-[10px] md:text-xs text-gray-600">{{ $index + 1 }}</span>
                        </div>
                    @endforeach
                </div>
                <p class="mt-3 md:mt-4 text-xs md:text-sm text-center text-gray-500">Dernières 10 notes (ordre chronologique inverse)</p>
            </div>
        </div>
        @endif
    </div>
...
/* Personnalisation de la pagination responsive */
.pagination {
    display: flex;
    gap: 0.25rem;
    flex-wrap: wrap;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const appreciations = document.querySelectorAll('.max-w-xs .group, .max-w-\\[150px\\] .group');
