    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-blue-900 rounded-2xl shadow-2xl mb-8 overflow-hidden relative group">
                ...
                <div
                    class="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -translate-y-32 translate-x-32 group-hover:translate-y-0 group-hover:translate-x-0 transition-transform duration-700">
                </div>
                <div
                    class="absolute bottom-0 left-0 w-48 h-48 bg-white/5 rounded-full translate-y-24 -translate-x-24 group-hover:translate-y-0 group-hover:translate-x-0 transition-transform duration-700">
                </div>

                <div class="relative px-8 py-6">
                    <div class="flex items-center space-x-4">
                        <div
                            class="bg-white/20 p-3 rounded-xl backdrop-blur-sm group-hover:scale-110 transition-transform duration-300">
            ...
            <!-- Messages flash -->
            @if (session('success'))
                <div class="mb-6 bg-green-100 border-l-4 border-green-500 text-green-700 p-4 rounded-r-lg shadow-md animate-slideDown"
                    role="alert">
            ...
            @if (session('error'))
                <div class="mb-6 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-r-lg shadow-md animate-slideDown"
                    role="alert">
                    <div class="flex items-center">
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                @if ($enseignant->statut)
                                    <span
                                        class="px-3 py-1.5 inline-flex text-xs font-semibold rounded-full bg-green-100 text-green-800 border border-green-200">
                                        <span class="w-2 h-2 bg-green-500 rounded-full mr-2 animate-pulse"></span>
                                        Actif
                                    </span>
                                @else
                                    <span
                                        class="px-3 py-1.5 inline-flex text-xs font-semibold rounded-full bg-red-100 text-red-800 border border-red-200">
                ...
                <!-- Corps du formulaire -->
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.enseignants.update', $enseignant) }}" class="space-y-6"
                        ...
                        <div class="mb-6">
                            ...
                            <div class="flex items-center space-x-6">
                                <div class="flex-shrink-0">
                                    <div class="w-20 h-20 rounded-xl bg-blue-900 flex items-center justify-center border-2 border-gray-300 overflow-hidden shadow-md"
                                        id="photo-preview-container">
                                        @if ($enseignant->photo)
                                            <img src="{{ Storage::url($enseignant->photo) }}"
                                                alt="{{ $enseignant->prenom }}" class="w-full h-full object-cover">
                                        @else
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">Laissez vide pour conserver la photo actuelle.
                                        Formats acceptés : JPEG, PNG, JPG, GIF (max. 2 Mo)</p>
                                    @error('photo')
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    ...
                                    <div class="mt-2 text-sm text-yellow-700">
                                        <p>Le matricule ({{ $enseignant->matricule }}) est généré automatiquement et ne
                                            peut pas être modifié. L'âge est calculé automatiquement à partir de la date de
                                            naissance.</p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="relative group">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        <svg class="h-5 w-5 text-gray-400 group-focus-within:text-blue-500 transition-colors"
                                            ...
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M3 10h18M3 14h18M3 18h18"></path>
                                        </svg>
                                    </div>
                            ...
                            <div>
                                ...
                                <div class="relative group">
                                    ...
                                    <select name="genre" id="genre" required
                                        ...
                                        <option value="m"
                                            {{ strtolower(old('genre', $enseignant->genre)) == 'm' ? 'selected' : '' }}>
                                            Masculin</option>
                                        <option value="f"
                                            {{ strtolower(old('genre', $enseignant->genre)) == 'f' ? 'selected' : '' }}>
                                            Féminin</option>
                                    </select>
                            ...
                            <div>
                                ...
                                <div class="relative group">
                                    ...
                                    <input type="date" name="date_naissance" id="date_naissance"
                                        value="{{ old('date_naissance', $enseignant->date_naissance?->format('Y-m-d')) }}"
                                        required
                            ...
                            <div>
                                ...
                                <div class="relative group">
                                    ...
                                    <input type="tel" name="telephone" id="telephone"
                                        value="{{ old('telephone', $enseignant->telephone) }}" inputmode="numeric"
                                        pattern="[0-9\s\+\-]{6,20}"
                            ...
                            <div>
                                ...
                                <div class="relative group">
                                    ...
                                    <select name="statut" id="statut"
                                        ...
                                        <option value="1"
                                            {{ old('statut', $enseignant->statut) == 1 ? 'selected' : '' }}>Actif</option>
                                        <option value="0"
                                            {{ old('statut', $enseignant->statut) == 0 ? 'selected' : '' }}>Inactif
                                        </option>
                                    </select>
                                    <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                        <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor"
                                ...
                                <p class="mt-1 text-xs text-gray-500">Un enseignant inactif ne peut pas être assigné à de
                                    nouvelles classes</p>
                                @error('statut')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            @foreach ($enseignant->enseignantMatiereClasses as $assignation)
                                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                    <div class="flex items-center justify-between mb-2">
                                        <span
                                            class="text-sm font-semibold text-blue-900">{{ $assignation->matiere->nom ?? 'Matière' }}</span>
                                        <span
                                            class="text-xs px-2 py-1 bg-blue-100 text-indigo-800 rounded-full">{{ $assignation->classe->nom ?? 'Classe' }}</span>
                                    </div>
                                    <p class="text-xs text-gray-500">Année scolaire:
            ...
            if (form) {
                form.addEventListener('submit', function(e) {
                    ...
                    if (!isValid) {
                        ...
                        toast.className =
                            'fixed top-4 right-4 bg-red-100 border-l-4 border-red-500 text-red-700 p-4 rounded-lg shadow-lg z-50 animate-slideDown';
                        toast.innerHTML = `
                        <div class="flex items-center">
