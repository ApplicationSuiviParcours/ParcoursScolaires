...
@section('header')
    <div class="relative overflow-hidden bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 py-8 md:py-12">
        ...
        <div class="absolute inset-0 opacity-10">
            ...
            <div
                class="absolute -bottom-24 -left-24 w-96 h-96 bg-indigo-300 rounded-full mix-blend-overlay filter blur-3xl animate-pulse delay-1000">
            </div>
        </div>
        ...
        <div class="container mx-auto px-4 relative z-10">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                <div class="text-center lg:text-left mb-6 lg:mb-0">
                    <nav class="flex mb-4 justify-center lg:justify-start" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1 md:space-x-3 flex-wrap">
                            <li class="inline-flex items-center">
                                <a href="{{ route('admin.parents.index') }}"
                                    class="inline-flex items-center text-sm font-medium text-blue-200 hover:text-white transition-colors duration-300">
                            ...
                            <li aria-current="page">
                                <div class="flex items-center">
                                    ...
                                    <span
                                        class="ml-1 text-sm font-medium text-white md:ml-2 truncate max-w-[150px] sm:max-w-none">{{ $parent->full_name }}</span>
                                </div>
                            </li>
                        </ol>
                    </nav>
                    <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2 animate-fade-in-up truncate">
                        {{ $parent->full_name }}
                    </h1>
                    <p class="text-blue-200 text-sm md:text-lg animate-fade-in-up animation-delay-200">
                        {{ $parent->profession ?? 'Parent d\'élève' }}
                    </p>
                </div>
                <div
                    class="flex flex-col sm:flex-row justify-center lg:justify-end space-y-3 sm:space-y-0 sm:space-x-3 animate-fade-in-right">
                    <a href="{{ route('admin.parents.edit', $parent) }}"
                        class="group relative inline-flex items-center justify-center px-4 py-2.5 bg-yellow-500 hover:bg-yellow-600 text-white font-medium rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-xl overflow-hidden">
                        <span
                    ...
                    <a href="{{ route('admin.parents.index') }}"
                        class="group relative inline-flex items-center justify-center px-4 py-2.5 bg-white/10 backdrop-blur-lg hover:bg-white/20 text-white font-medium rounded-xl transition-all duration-300 transform hover:scale-105 border border-white/20">
...
@section('content')
    <div class="container mx-auto px-4 py-6 md:py-10 bg-gray-50">
        <!-- Badges de statut flottants (Masqués sur mobile pour éviter les chevauchements) -->
            ...
            <div class="absolute -top-12 right-0 flex space-x-2">
                ...
                @if($parent->statut !== null)
                    <div class="bg-white rounded-full shadow-xl px-6 py-3 flex items-center space-x-3">
                        ...
                        <span
                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $parent->statut ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8 mb-8">
            ...
            <div class="bg-white rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden">
                ...
                <div class="p-4 md:p-6">
                    <div class="relative inline-block mb-6 w-full flex justify-center">
                        @if($parent->photo)
                            <img src="{{ Storage::url($parent->photo) }}" alt="{{ $parent->full_name }}"
                                class="w-28 h-28 md:w-32 md:h-32 rounded-2xl object-cover shadow-lg mx-auto transform hover:scale-110 transition-transform duration-300">
                        @else
                            <div
                                class="w-28 h-28 md:w-32 md:h-32 {{ $parent->avatar_color }} rounded-2xl flex items-center justify-center text-white font-bold text-4xl shadow-lg mx-auto transform hover:scale-110 transition-transform duration-300">
                                {{ $parent->initiales }}
                            </div>
                        @endif
                        @if($parent->hasUserAccount())
                            <div
                                class="absolute -bottom-2 -right-2 w-6 h-6 bg-green-400 border-4 border-white rounded-full animate-pulse">
                            </div>
                        @endif
                    </div>

                    <h2 class="text-xl md:text-2xl font-bold text-gray-800 text-center truncate">{{ $parent->full_name }}
                    </h2>
                    <p class="text-gray-500 text-center text-sm md:text-base truncate">
                        {{ $parent->profession ?? 'Profession non renseignée' }}</p>
                        ...
                        <div class="grid grid-cols-2 gap-3 text-xs md:text-sm">
                            @if($parent->date_naissance)
                                <div>
                                    ...
                                    <p class="font-medium">{{ $parent->date_naissance->format('d/m/Y') }}</p>
                                </div>
                            @endif
                            @if($parent->lieu_naissance)
                                <div>
                                    <p class="text-gray-500">Lieu naissance</p>
                                    <p class="font-medium truncate">{{ $parent->lieu_naissance }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    @if($parent->email)
                        <div
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="font-medium text-gray-800 truncate">{{ $parent->email }}</p>
                            </div>
                        </div>
                    @endif
                    ...
                    @if($parent->adresse)
                        <div
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="font-medium text-gray-800 text-sm">{{ $parent->adresse }}</p>
                            </div>
                        </div>
                    @endif

                    <!-- Réseaux sociaux -->
                    @if($parent->facebook || $parent->whatsapp)
                    ...
                    @if($parent->user)
                        <div class="flex items-center mb-4">
                            ...
                            <div class="min-w-0">
                                ...
                                <p class="text-lg font-bold text-gray-800 truncate">{{ $parent->user->name }}</p>
                                <p class="text-sm text-gray-600 truncate">{{ $parent->user->email }}</p>
                            </div>
                        </div>
                        <div class="border-t border-gray-200 pt-4 mt-2 space-y-2 text-sm">
                            <div class="flex justify-between">
                                <span class="text-gray-500">Créé le</span>
                                <span class="font-medium text-right">{{ $parent->user->created_at->format('d/m/Y') }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Dernière connexion</span>
                                <span
                                    class="font-medium text-right">{{ $parent->user->last_login_at ? $parent->user->last_login_at->format('d/m/Y') : 'Jamais' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Rôle</span>
                                <span
                                    class="font-medium text-right">{{ $parent->user->roles->pluck('name')->join(', ') ?: 'Parent' }}</span>
                            </div>
                        </div>
                    @else
                        <div class="text-center py-6">
                            ...
                            <p class="text-gray-500 mb-2 text-sm md:text-base">Aucun compte utilisateur associé</p>
                            <p class="text-xs md:text-sm text-gray-400 mb-4">Ce parent n'a pas encore de compte pour se
                                connecter</p>
                            <a href="{{ route('admin.users.create', ['parent_id' => $parent->id]) }}"
                                class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-all duration-300 transform hover:scale-105 text-sm">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xl md:text-2xl font-bold text-gray-800">
                            {{ $parent->eleves->sum(fn($e) => $e->notes->count()) }}</p>
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-4 border-l-4 border-yellow-500">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xl md:text-2xl font-bold text-gray-800">
                            {{ $parent->eleves->sum(fn($e) => $e->absences->count()) }}</p>
                    </div>
            ...
            <div class="bg-gradient-to-r from-amber-500 to-orange-600 px-4 md:px-8 py-5">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex items-center mb-3 sm:mb-0">
                        <div
                            class="w-10 h-10 md:w-12 md:h-12 bg-white/20 backdrop-blur-lg rounded-xl flex items-center justify-center mr-4">
                        ...
                        <h2 class="text-lg md:text-xl font-bold text-white">Enfants associés</h2>
                    </div>
                    <span
                        class="px-4 py-2 bg-white/20 backdrop-blur-lg text-white rounded-xl text-sm font-medium self-start sm:self-auto">
            ...
            <div class="p-4 md:p-6">
                                        ...
                                        <div class="mt-2 space-y-1">
                                            @if($eleve->classe)
                                                <p class="text-xs md:text-sm text-gray-600 flex items-center">
                                                    <svg class="w-4 h-4 mr-1 text-gray-500 flex-shrink-0" fill="none"
                                                        stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                            d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"></path>
                                                    </svg>
                                                    <span class="truncate">{{ $eleve->classe->nom }}</span>
                                                </p>
                                            @endif

                                            @if($eleve->pivot && $eleve->pivot->lien_parental)
                                                <span
                                                    class="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-blue-100 text-blue-700 rounded-full">
                                            ...
                                            <div class="mt-3 pt-3 border-t border-gray-200">
                                                ...
                                                <div class="flex flex-wrap gap-2">
                                                    @foreach($eleve->notes->take(3) as $note)
                                                        <span class="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-medium rounded-lg">
                                                            {{ $note->note }}/20
                                                        </span>
                                                    @endforeach
                                                </div>
                ...
                @else
                    <div class="text-center py-8 md:py-12 px-4">
                        <div
                            class="w-16 h-16 md:w-20 md:h-20 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4 animate-pulse">
                        ...
                        <p class="text-gray-500 text-base md:text-lg mb-2">Aucun enfant associé</p>
                        <p class="text-xs md:text-sm text-gray-400 mb-4">Ce parent n'a pas encore d'enfants inscrits</p>
                        <a href="{{ route('admin.parents.edit', $parent) }}"
                            class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-all duration-300 transform hover:scale-105 text-sm">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white rounded-3xl shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300">
                    ...
                    <div class="p-4 md:p-6">
                        <div class="bg-gray-50 rounded-xl p-4">
                            <p class="text-gray-700 whitespace-pre-line leading-relaxed text-sm">{{ $parent->notes }}</p>
                        </div>
                        <p class="text-xs text-gray-500 mt-3 flex items-center">
                            ...
                            Modifié : {{ $parent->updated_at->format('d/m/Y H:i') }}
                        </p>
                    </div>
                </div>
            @endif
                ...
                <div class="p-4 md:p-6">
                    <div class="space-y-3 text-sm">
                        <div
                            ...
                            <span
                                class="font-medium text-gray-800 mt-1 sm:mt-0">{{ $parent->created_at->format('d/m/Y à H:i:s') }}</span>
                        </div>
                        <div
                            class="flex flex-col sm:flex-row sm:justify-between sm:items-center py-2 border-b border-gray-100">
                            <span class="text-gray-600 font-medium">Dernière mise à jour</span>
                            <span
                                class="font-medium text-gray-800 mt-1 sm:mt-0">{{ $parent->updated_at->format('d/m/Y à H:i:s') }}</span>
                        </div>
                        <div
                            class="flex flex-col sm:flex-row sm:justify-between sm:items-center py-2 border-b border-gray-100">
                            <span class="text-gray-600 font-medium">ID unique</span>
                            <span
                                class="font-mono text-sm bg-gray-100 px-3 py-1 rounded-lg text-gray-800 mt-1 sm:mt-0">#{{ $parent->id }}</span>
                        </div>
                        @if($parent->created_by)
                            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center py-2">
                                ...
                                <span
                                    class="font-medium text-gray-800 mt-1 sm:mt-0">{{ $parent->creator->name ?? 'Système' }}</span>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="mt-8 flex flex-col sm:flex-row justify-center sm:justify-end space-y-3 sm:space-y-0 sm:space-x-4">
            ...
            <a href="{{ route('admin.parents.index') }}"
                class="w-full sm:w-auto px-6 py-3 bg-white border-2 border-gray-300 text-gray-700 font-medium rounded-xl hover:bg-gray-50 transition-all duration-300 transform hover:scale-105 flex items-center justify-center">
...
@push('styles')
    <style>
        /* Vos animations existentes conservées */
        @keyframes float-1 {
        ...
        /* Amélioration pour le scroll horizontal sur mobile */
        .overflow-x-auto {
