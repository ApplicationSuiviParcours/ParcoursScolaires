<x-guest-layout>
    ...
    {{-- ÉTAPE 1 : Connexion Utilisateur (Matricule) --}}
<div id="user-login" class="transition-all duration-300">
...
<!-- Test matricules removed per user request - ELE-2024-P0001 etc. -->
            </div>
    ...
    {{-- ÉTAPE 2 : Connexion Administrateur (Email + Mot de passe) --}}
    <div id="admin-login" class="hidden transition-all duration-300">
        <div class="mb-5 text-center">
            <div class="relative inline-flex items-center justify-center mb-3">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4" x-data="{ showPassword: false }">
            ...
            <!-- Se souvenir de moi -->
            <div class="flex items-center">
                <input id="remember_me_admin" type="checkbox" name="remember" class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 cursor-pointer">
                <label for="remember_me_admin" class="ml-2.5 block text-sm font-medium text-gray-600 cursor-pointer select-none">
                    Se souvenir de moi
                </label>
            </div>
            ...
            <div class="flex items-center justify-between pt-2 border-t border-gray-100">
                ...
                <a href="{{ route('login') }}" class="text-sm font-medium text-gray-500 hover:text-blue-600 hover:underline">
                    Retour à la connexion principale &rsaquo;
                </a>
            </div>
        </form>
    </div>
    ...
    @once
        @push('scripts')
            ...
            <script>
                ...
                // Si on revient avec une erreur admin, rester sur le panneau admin
                @if(session('login_type') === 'admin' || old('role') === 'administrateur' || $errors->has('password'))
                    document.addEventListener('DOMContentLoaded', switchToAdmin);
                @endif
            </script>
        @endpush
    @endonce
