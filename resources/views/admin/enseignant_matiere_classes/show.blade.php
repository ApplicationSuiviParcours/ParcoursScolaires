                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li aria-current="page">
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-sm font-medium text-white md:ml-2">Détails #{{ $enseignantMatiereClasse->id }}</span>
                            </div>
                        </li>
                    </ol>
                </nav>
        ...
        <div class="bg-white rounded-2xl md:rounded-3xl shadow-xl overflow-hidden mb-6 md:mb-8">
            <div class="bg-blue-900 px-4 md:px-8 py-4 md:py-6">
                <div class="flex items-center">
                    ...
                    <div>
                        <h2 class="text-xl md:text-2xl font-bold text-white mb-1">Information détaillée</h2>
                        <p class="text-blue-100 text-xs md:text-sm">Affectation #{{ $enseignantMatiereClasse->id }}</p>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
                    ...
                    <div class="space-y-4 md:space-y-6">
                        ...
                        <div class="bg-gray-50 rounded-xl md:rounded-2xl p-4 md:p-5">
                            ...
                            @if($enseignantMatiereClasse->enseignant)
                                <div class="flex flex-col sm:flex-row items-center text-center sm:text-left">
                                    <div class="flex-shrink-0 h-12 w-12 md:h-14 md:w-14 bg-blue-900 rounded-xl flex items-center justify-center text-white font-bold text-lg md:text-xl shadow-lg mb-3 sm:mb-0 sm:mr-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs md:text-sm text-gray-600">Spécialité: {{ $enseignantMatiereClasse->enseignant->specialite ?? 'Général' }}</p>
                                        <p class="text-xs md:text-sm text-gray-600 hidden sm:block">Matricule: {{ $enseignantMatiereClasse->enseignant->matricule ?? 'N/A' }}</p>
                                    </div>
                                </div>
                            @else
                        ...
                        <div class="bg-gray-50 rounded-xl md:rounded-2xl p-4 md:p-5">
                            ...
                            @if($enseignantMatiereClasse->classe)
                                <div class="flex flex-col sm:flex-row items-center text-center sm:text-left">
                                    <div class="w-10 h-10 md:w-12 md:h-12 bg-green-100 rounded-lg md:rounded-xl flex items-center justify-center mb-3 sm:mb-0 sm:mr-3 flex-shrink-0">
                                        <span class="text-green-700 font-bold text-base md:text-lg">{{ substr($enseignantMatiereClasse->classe->nom, 0, 3) }}</span>
                                    </div>
                                    <div>
                                        <p class="text-base md:text-lg font-bold text-gray-800">{{ $enseignantMatiereClasse->classe->nom }}</p>
                                        <p class="text-xs md:text-sm text-gray-600">Capacité: {{ $enseignantMatiereClasse->classe->capacite ?? 'N/A' }} élèves</p>
                                    </div>
                                </div>
                            @else
                    ...
                    <div class="space-y-4 md:space-y-6">
                        ...
                        <div class="bg-gray-50 rounded-xl md:rounded-2xl p-4 md:p-5">
                            ...
                            @if($enseignantMatiereClasse->matiere)
                                <div class="flex flex-col sm:flex-row items-center text-center sm:text-left">
                                    <div class="w-10 h-10 md:w-12 md:h-12 bg-blue-100 rounded-lg md:rounded-xl flex items-center justify-center mb-3 sm:mb-0 sm:mr-3 flex-shrink-0">
                                        <span class="text-blue-900 font-bold text-sm md:text-base">{{ substr($enseignantMatiereClasse->matiere->code ?? 'MT', 0, 3) }}</span>
                                    </div>
                                    <div>
                                        <p class="text-base md:text-lg font-bold text-gray-800">{{ $enseignantMatiereClasse->matiere->nom }}</p>
                                        <p class="text-xs md:text-sm text-gray-600">Code: {{ $enseignantMatiereClasse->matiere->code ?? 'N/A' }}</p>
                                        <p class="text-xs md:text-sm text-gray-600 hidden sm:block">Coefficient: {{ $enseignantMatiereClasse->matiere->coefficient ?? 1 }}</p>
                                    </div>
                                </div>
                            @else
                        ...
                        <div class="bg-gray-50 rounded-xl md:rounded-2xl p-4 md:p-5">
                            ...
                            @if($enseignantMatiereClasse->anneeScolaire)
                                <div class="flex flex-col sm:flex-row items-center text-center sm:text-left">
                                    <div class="w-10 h-10 md:w-12 md:h-12 bg-yellow-100 rounded-lg md:rounded-xl flex items-center justify-center mb-3 sm:mb-0 sm:mr-3 flex-shrink-0">
                                        <svg class="w-5 h-5 md:w-6 md:h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                        </svg>
                                    </div>
                                    <div>
                                        <p class="text-base md:text-lg font-bold text-gray-800">{{ $enseignantMatiereClasse->anneeScolaire->nom }}</p>
                                        <p class="text-xs md:text-sm text-gray-600">Créée le {{ $enseignantMatiereClasse->created_at->format('d/m/Y') }}</p>
                                    </div>
                                </div>
                            @else
        ...
        <div class="bg-white rounded-2xl md:rounded-3xl shadow-xl overflow-hidden mb-6 md:mb-8">
            <div class="bg-blue-900 px-4 md:px-8 py-4 md:py-5">
                <div class="flex items-center">
                    ...
                    <h2 class="text-lg md:text-xl font-bold text-white">Autres affectations</h2>
                </div>
            </div>
            <div class="p-4 md:p-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                    @foreach($autresAffectations as $affectation)
                        <a href="{{ route('admin.enseignant_matiere_classes.show', $affectation) }}" class="block p-3 md:p-4 bg-gray-50 rounded-lg md:rounded-xl hover:bg-gray-100 transition-all duration-300">
                            <div class="flex items-center">
                                <div class="w-8 h-8 md:w-10 md:h-10 bg-blue-100 rounded-lg flex items-center justify-center mr-2 md:mr-3 flex-shrink-0">
                                    <span class="text-blue-900 font-bold text-[10px] md:text-sm">{{ substr($affectation->matiere->code ?? 'MT', 0, 3) }}</span>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="font-medium text-gray-800 text-xs md:text-sm truncate">{{ $affectation->matiere->nom ?? 'N/A' }}</p>
                                    <p class="text-[10px] md:text-xs text-gray-500 truncate">{{ $affectation->classe->nom ?? 'N/A' }} - {{ $affectation->anneeScolaire->nom ?? 'N/A' }}</p>
                                </div>
