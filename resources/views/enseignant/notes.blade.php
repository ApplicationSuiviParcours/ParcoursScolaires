...
@section('content')
    <div class="py-6 sm:py-12">
        <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
            ...
            <div class="bg-gradient-to-r from-green-700 to-green-500 rounded-lg sm:rounded-xl shadow-lg mb-6 sm:mb-8 p-4 sm:p-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h3 class="text-xl sm:text-2xl font-bold text-white">Saisie des Notes</h3>
                        <p class="text-green-100 text-sm sm:text-base mt-0.5 sm:mt-1">Enregistrer, modifier et gérer les notes des élèves</p>
                    </div>
                    <div class="flex items-center justify-between sm:justify-end gap-3 sm:gap-4">
                        <a href="{{ route('enseignant.notes.create') }}"
                            class="bg-white text-green-600 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg font-semibold hover:bg-green-50 transition-colors flex items-center text-sm sm:text-base whitespace-nowrap">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
                <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-3 sm:p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs font-medium text-gray-500">Total notes</p>
                            <p class="text-xl sm:text-2xl font-bold text-gray-900">{{ $totalNotes ?? 0 }}</p>
                        </div>
                ...
                <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-3 sm:p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs font-medium text-gray-500">Moyenne générale</p>
                            <p class="text-xl sm:text-2xl font-bold text-gray-900">{{ $moyenneGenerale ?? 0 }}/20</p>
                        </div>
                ...
                <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-3 sm:p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xl sm:text-2xl font-bold text-gray-900">{{ $classesCount ?? 0 }}</p>
                        </div>
                ...
                <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-3 sm:p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-xs font-medium text-gray-500">Évaluations</p>
                            <p class="text-xl sm:text-2xl font-bold text-gray-900">{{ $evaluationsCount ?? 0 }}</p>
                        </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    <div>
                        ...
                        <select id="filter_classe" 
                            class="w-full px-2 py-1.5 sm:px-3 sm:py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500">
                    ...
                    <div>
                        <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">Matière</label>
                    ...
                    <div>
                        <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">Évaluation</label>
                    ...
                    <div>
                        <label class="block text-xs sm:text-sm font-medium text-gray-700 mb-1 sm:mb-2">Élève</label>
                        <input type="text" id="filter_eleve" placeholder="Rechercher un élève..."
                            class="w-full px-2 py-1.5 sm:px-3 sm:py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500">
                    </div>
                </div>
                <div class="flex flex-col sm:flex-row justify-end mt-3 sm:mt-4 space-y-2 sm:space-y-0 sm:space-x-2">
                    <button onclick="reinitialiserFiltres()"
                        class="px-3 py-1.5 sm:px-4 sm:py-2 text-sm sm:text-base bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                        Réinitialiser
                    </button>
                    <button onclick="appliquerFiltres()"
                        class="px-3 py-1.5 sm:px-4 sm:py-2 text-sm sm:text-base bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors">
                        Appliquer les filtres
                    </button>
                </div>
            </div>

            <!-- Notes Table avec toutes les colonnes -->
            <div class="bg-white rounded-lg sm:rounded-xl shadow-md overflow-hidden">
                <div class="p-4 sm:p-6 border-b border-gray-100">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                        <h3 class="text-base sm:text-lg font-semibold text-gray-900">Liste des notes</h3>
                        <span class="text-xs sm:text-sm text-gray-500">Total: {{ $notes->total() ?? $notes->count() }} note(s)</span>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto" style="-webkit-overflow-scrolling: touch;">
                    <table class="w-full border-collapse" id="notesTable">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">N°</th>
                                ...
                                <th class="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap min-w-[150px]">Évaluation</th>
                                ...
                                <th class="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Type</th>
                                <th class="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">Note</th>
                                <th class="px-2 sm:px-3 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap min-w-[150px]">Observation</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($notes as $index => $note)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap text-xs sm:text-sm text-gray-500">
                                        {{ $index + 1 }}
                                    </td>
                                    ...
                                    <td class="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap">
                                        <span class="px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full {{ $note->note >= 10 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }} whitespace-nowrap">
                                            {{ $note->note }}/20
                                        </span>
                                    </td>
                                    <td class="px-2 sm:px-3 py-2 sm:py-3 text-xs sm:text-sm text-gray-500 max-w-[200px] truncate">
                                        {{ $note->observation ?? '-' }}
                                    </td>
                                    <td class="px-2 sm:px-3 py-2 sm:py-3 whitespace-nowrap">
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="px-3 sm:px-6 py-8 sm:py-12 text-center text-gray-500">
                                        <svg class="w-12 h-12 sm:w-16 sm:h-16 mx-auto text-gray-400 mb-3 sm:mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2">
                                            </path>
                                        </svg>
                                        <p class="text-sm sm:text-lg">Aucune note trouvée</p>
                                        <p class="text-xs sm:text-sm mt-1 sm:mt-2">Commencez par saisir une note</p>
                                        <a href="{{ route('enseignant.notes.create') }}"
                                            class="mt-3 sm:mt-4 inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 text-sm sm:text-base bg-green-600 text-white rounded-lg hover:bg-green-700">
    ...
    <div id="modalSuppression" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
        <div class="relative top-20 mx-auto p-4 sm:p-5 border w-full max-w-md shadow-lg rounded-lg bg-white">
            <div class="text-center">
                ...
                <h3 class="text-base sm:text-lg font-medium text-gray-900 mb-1 sm:mb-2">Confirmer la suppression</h3>
                <p class="text-xs sm:text-sm text-gray-500 mb-3 sm:mb-4">Êtes-vous sûr de vouloir supprimer cette note ? Cette action est irréversible.</p>
                <div class="flex justify-center space-x-2 sm:space-x-3">
                    ...
                    <button onclick="supprimerNote()"
                        class="px-3 py-1.5 sm:px-4 sm:py-2 text-sm sm:text-base bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
                        Supprimer
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        if (filterEleve) {
            filterEleve.addEventListener('keyup', function () {
                ...
                lignes.forEach(ligne => {
                    ...
                    const eleve = cells[1]?.textContent.toLowerCase() || '';
                    const classe = cells[2]?.textContent.toLowerCase() || '';
                    const evaluation = cells[3]?.textContent.toLowerCase() || '';
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (urlParams.has('classe')) {
                ...
                if (classeSelect) classeSelect.value = urlParams.get('classe');
            }
            if (urlParams.has('matiere')) {
                const matiereSelect = document.getElementById('filter_matiere');
                if (matiereSelect) matiereSelect.value = urlParams.get('matiere');
            }
            if (urlParams.has('evaluation')) {
                const evaluationSelect = document.getElementById('filter_evaluation');
                if (evaluationSelect) evaluationSelect.value = urlParams.get('evaluation');
            }
            if (urlParams.has('eleve')) {
                const eleveInput = document.getElementById('filter_eleve');
                if (eleveInput) eleveInput.value = urlParams.get('eleve');
            }
        });
    </script>
