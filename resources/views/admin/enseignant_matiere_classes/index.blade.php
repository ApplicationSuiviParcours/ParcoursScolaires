        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex justify-center md:justify-end animate-fade-in-right">
                <a href="{{ route('admin.enseignant_matiere_classes.create') }}" 
                   class="group relative inline-flex items-center justify-center px-5 py-2.5 md:px-6 md:py-3 bg-white/20 backdrop-blur-lg hover:bg-white/30 text-white font-semibold rounded-xl transition-all duration-300 transform hover:scale-105 border border-white/30 text-sm">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
        <div class="bg-white rounded-xl md:rounded-2xl shadow-lg p-4 md:p-6 border-l-4 border-indigo-500 hover:shadow-xl transition-all duration-300">
            ...
            <div class="mt-2 text-xs text-gray-500">
                <span class="font-medium">{{ $stats['total_enseignants'] }}</span> enseignants
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl md:rounded-2xl shadow-lg p-4 md:p-6 border-l-4 border-green-500 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl md:text-3xl font-bold text-gray-800">{{ $stats['total_classes'] }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl md:rounded-2xl shadow-lg p-4 md:p-6 border-l-4 border-blue-500 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-gray-500 font-medium uppercase">Matières</p>
        ...
        <div class="bg-white rounded-xl md:rounded-2xl shadow-lg p-4 md:p-6 border-l-4 border-yellow-500 hover:shadow-xl transition-all duration-300">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-gray-500 font-medium uppercase">Années</p>
                    <p class="text-xl md:text-3xl font-bold text-gray-800">{{ $stats['total_annees'] }}</p>
                </div>
                    ...
                    @forelse($affectations as $affectation)
                        <tr class="hover:bg-indigo-50/50 transition-colors duration-200 group">
                            ...
                            <td class="px-3 md:px-6 py-3 md:py-4">
                                <div class="flex items-center">
                                    <div class="w-7 h-7 md:w-8 md:h-8 bg-blue-100 rounded-md flex items-center justify-center mr-1 md:mr-2 flex-shrink-0">
                                        <span class="text-blue-700 font-bold text-[10px] md:text-xs">{{ substr($affectation->matiere->code ?? 'MT', 0, 3) }}</span>
                                    </div>
                                    <span class="text-xs md:text-sm font-medium text-gray-900 truncate">{{ $affectation->matiere->nom }}</span>
                                </div>
                            </td>
                            <td class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap">
                                <span class="px-2 md:px-3 py-0.5 md:py-1 bg-green-100 text-green-700 text-[10px] md:text-sm font-medium rounded-full">
                            ...
                            <td class="px-3 md:px-6 py-3 md:py-4 whitespace-nowrap">
                                <div class="flex items-center justify-end gap-1 md:gap-2 whitespace-nowrap">
                                    ...
                                    <form action="{{ route('admin.enseignant_matiere_classes.destroy', $affectation) }}" method="POST" class="inline m-0 p-0 delete-form" onsubmit="return confirm('Supprimer cette affectation ?');">
                                        @csrf @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12">
                                <div class="flex flex-col items-center justify-center text-center">
                                    ...
                                    <h3 class="text-lg md:text-xl font-bold text-gray-800 mb-2">Aucune affectation</h3>
        ...
        @if($affectations->hasPages())
            <div class="px-4 md:px-6 py-3 md:py-4 border-t border-gray-200 bg-gray-50 flex flex-col sm:flex-row items-center justify-between gap-3">
                <div class="text-xs md:text-sm text-gray-600 order-2 sm:order-1">
                    ...
                    <span class="font-medium">{{ $affectations->total() }}</span>
                </div>
...
<style>
    ...
    .pagination .page-item.active .page-link { background: linear-gradient(to right, #6366f1, #8b5cf6); color: white; border-color: transparent; }
