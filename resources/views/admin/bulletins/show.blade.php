        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li aria-current="page">
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-sm font-medium text-white md:ml-2 truncate max-w-[150px] md:max-w-none">
                                    {{ $bulletin->eleve->nom }}
                                </span>
                            </div>
                        </li>
            ...
            <div class="flex flex-col sm:flex-row justify-center md:justify-end space-y-3 sm:space-y-0 sm:space-x-3 animate-fade-in-right">
                ...
                <a href="{{ route('admin.bulletins.print', $bulletin) }}"
                   class="group relative inline-flex items-center justify-center px-4 py-2.5 md:px-5 bg-green-500 hover:bg-green-600 text-white font-medium rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-xl overflow-hidden text-sm"
                   target="_blank">
                    <span class="absolute inset-0 bg-white opacity-0 group-hover:opacity-20 transition-opacity duration-300"></span>
                    <svg class="w-5 h-5 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-2xl md:rounded-3xl shadow-xl overflow-hidden">
            ...
            <div class="p-4 md:p-6">
                <div class="flex flex-col sm:flex-row items-center text-center sm:text-left">
                    ...
                    <div>
                        <h2 class="text-lg md:text-xl font-bold text-gray-800">{{ $bulletin->eleve->nom }} {{ $bulletin->eleve->prenom }}</h2>
                        <p class="text-xs md:text-sm text-gray-600">Matricule: {{ $bulletin->eleve->matricule ?? 'N/A' }}</p>
                        <p class="text-xs md:text-sm text-gray-600">Né(e) le: {{ $bulletin->eleve->date_naissance?->format('d/m/Y') ?? 'N/A' }}</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-3">
                    <div class="flex items-center">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="font-medium text-gray-800 truncate">{{ $bulletin->classe->nom }}</p>
                        </div>
                    </div>
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="font-medium text-gray-800 truncate">{{ $bulletin->anneeScolaire->nom }}</p>
                        </div>
                    </div>
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="font-medium text-gray-800 truncate">{{ $bulletin->periode }}</p>
                        </div>
                    </div>
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="font-medium text-gray-800 truncate">{{ $bulletin->date_bulletin ?->format('d/m/Y') ?? 'N/A' }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl md:rounded-3xl shadow-xl overflow-hidden md:col-span-2 xl:col-span-1">
            ...
            <div class="p-4 md:p-6">
                <div class="text-center mb-4">
                    <span class="text-xs md:text-sm text-gray-500">Moyenne générale</span>
                    <div class="text-4xl md:text-5xl font-bold {{ $stats['moyenne'] >= 10 ? 'text-green-600' : 'text-red-600' }}">
                        {{ number_format($stats['moyenne'], 2) }}
                        <span class="text-xl md:text-2xl text-gray-400">/20</span>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3 md:gap-4 mt-4">
                    ...
                    <div class="bg-gray-50 rounded-xl p-3 text-center">
                        ...
                        <p class="text-xs md:text-sm font-bold {{ $stats['moyenne'] >= 10 ? 'text-green-600' : 'text-red-600' }}">
                            {{ $stats['mention'] }}
                        </p>
                    </div>
                </div>
                @if($stats['est_admis'])
                    <div class="mt-4 bg-green-100 text-green-700 rounded-xl p-3 text-center font-medium text-sm md:text-base">
                        ✅ Admis(e)
                    </div>
                @else
                    <div class="mt-4 bg-red-100 text-red-700 rounded-xl p-3 text-center font-medium text-sm md:text-base">
                        ❌ Non admis(e)
                    </div>
                @endif
            </div>
        ...
        <div class="bg-gradient-to-r from-gray-800 to-gray-900 px-4 md:px-8 py-4 md:py-5">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div class="flex items-center">
                    ...
                    <h2 class="text-lg md:text-xl font-bold text-white">Détail des notes</h2>
                </div>
                <span class="px-3 md:px-4 py-2 bg-white/10 backdrop-blur-lg text-white rounded-lg md:rounded-xl text-xs md:text-sm font-medium self-start sm:self-auto">
        ...
        <div class="p-4 md:p-6">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Matière</th>
                                <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Notes</th>
                                <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Moyenne</th>
                                <th class="hidden md:table-cell px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Coef.</th>
                            </tr>
                        </thead>
                            ...
                            @foreach($notesParMatiere as $matiereId => $data)
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    <td class="px-3 md:px-6 py-4 whitespace-nowrap">
                                        ...
                                        @if($matiereCode)
                                            <div class="text-xs text-gray-500 hidden md:block">{{ $matiereCode }}</div>
                                        @endif
                                    </td>
                                        ...
                                        <div class="space-y-2">
                                            @forelse($notes as $note)
                                                @php
                                                    ...
                                                    $evalType = $evaluation ? ($evaluation->type ?? 'Eval') : 'Eval';
                                                @endphp
                                                <div class="flex items-center justify-between bg-gray-50 rounded-lg px-2 py-1.5 md:px-3 md:py-2">
                                                    <div class="flex items-center gap-1 md:gap-2">
                                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium {{ $noteValue >= 10 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                            ...
                                            @empty
                                                <span class="text-gray-400 text-xs md:text-sm">Aucune note</span>
                                            @endforelse
                                        </div>
                                    </td>
                                    <td class="px-3 md:px-6 py-4 whitespace-nowrap">
                                        <span class="text-base md:text-lg font-bold {{ $moyenne >= 10 ? 'text-green-600' : 'text-red-600' }}">
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 text-sm md:text-base">Aucune note disponible pour ce bulletin</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl md:rounded-3xl shadow-xl overflow-hidden mb-8">
        <div class="bg-gradient-to-r from-amber-500 to-orange-600 px-4 md:px-8 py-4 md:py-5">
            <div class="flex items-center">
                ...
                <h2 class="text-lg md:text-xl font-bold text-white">Appréciation</h2>
            </div>
        </div>
        <div class="p-4 md:p-8">
            <p class="text-gray-700 text-sm md:text-lg leading-relaxed">{{ $bulletin->appreciation }}</p>
        </div>
    </div>
    @endif
